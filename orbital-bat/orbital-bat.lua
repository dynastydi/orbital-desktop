...
function orbital_bat:init(args)
    self.ac = args.ac or "AC"
    self.adapter = args.adapter or "BAT0"
    self.ac_prefix = args.ac_prefix or "ߡϟ"
    self.battery_prefix = args.battery_prefix or "ߜϟ"
    ...
    self.alert_title = args.alert_title or "POWER LOW"
    self.alert_text = args.alert_text or "${time_est}\n${percent}%"
...
function orbital_bat:get_state()
    local pow   = "/sys/class/power_supply/"
...
function orbital_bat:notify(title, text)
    if self.alert then
...
return setmetatable(orbital_bat, {
    __call = orbital_bat.new,
})
